...
Метод `tick` (@code:tick) вызывается каждым таймером сцены (передаётся
...
При отпускании кнопки мыши после перетаскивания сущности на mouseReleaseEvent (@code:mouseReleaseEvent)
